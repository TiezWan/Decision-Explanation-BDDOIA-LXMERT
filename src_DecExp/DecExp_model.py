...
class DecExpModel(nn.Module):
    def __init__(self):
        ...
        self.actionpredict_head=nn.Linear(hid_dim * 2, 4)
        self.exppredict_head=nn.Linear(hid_dim * 2, ANSWER_LENGTH-4)
    ...
    def forward(self, imgid, feat, pos, sent):
        ...
        prelogit = self.logit_fc(x) #Does this return logits or label(logits followed by sigmoid) ? If latter -> Change loss in DecExp.py to BCELoss
