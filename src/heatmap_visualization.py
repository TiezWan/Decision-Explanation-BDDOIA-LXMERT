...
class HeatmapVisualization:
    ...
    def load_labels(self, ques_nr: int) -> Dict:
        """Load ground-truth bounding boxes of objects of interest for images based on different types of problems"""
    ...
    def compute_objs(
        ...
        """Compute the bounding boxes of objects of interest"""
        ...
        attention_scores = attention_scores / torch.sqrt(
            torch.tensor(self.num_heads, dtype=torch.float32)
        )
        ...
        if self.pooling == "CLS":
            ...
            np_source = np.array(
                normal_source[0] / max(normal_source[0])
        ...
        interestobjs = [
            ...
            if cluster_labels[idx] == cluster_max_idx
        ]
        ...
        if bboxcaplength != None:
            if len(interestobjs) > bboxcaplength:
                interestobjs = np.flip(np.argsort(np_source_ex1, 0))[:, 0][
                    :bboxcaplength
                ]  # Get the bboxcaplength with highest values
            ...
            if none_interestobjs:
                ...
                for obj in range(self.num_objs):
                    if (obj in num_interest_objs[img_nr]) and (obj in labels[imgid]):
                        true_pos += 1
                    elif (obj in num_interest_objs[img_nr]) and not (
                        obj in labels[imgid]
                    ):
                        false_pos += 1
                    elif not (obj in num_interest_objs[img_nr]) and (
                        obj in labels[imgid]
                    ):
                        false_neg += 1
                    elif not (obj in num_interest_objs[img_nr]) and not (
                        obj in labels[imgid]
                    ):
                        true_neg += 1
    ...
    def plot_bboxes(self, imgid, np_source, roi_idx, ques_nr: int) -> None:
        """Plot the bounding boxes on the original frame"""

        # load original frame
        img_path = os.path.join(
            self.orig_frame_path, "_".join(imgid.split("_")[:3]), imgid + ".jpg"
        )
        ...
        save_path = os.path.join(
            self.img_save_path, f"{imgid}_sent{ques_nr}_bboxes.jpg"
        )
    ...
    def run_and_eval(
        ...
        if sents is not None:
            for img_nr, img_id in enumerate(listimgids):
                ...
                interest_objs = self.compute_objs(
                    img_id,
                    attention_scores[img_nr, :, :, :],
                    sents[img_nr],
                    ques_nr,
                    cluster_lenth,
                    eps_dbscan,
                    plot=args.plot_heatmap,
                )
                interest_temp = self.pad_list(
                    np.array(interest_objs), self.num_objs, -1
                )  # Padding with -1 up to number of all objects
                num_interest_objs[img_nr] = np.array(interest_temp)

            true_pos, false_pos, false_neg, true_neg = self.evaluate(
                listimgids, num_interest_objs, ques_nr
            )
