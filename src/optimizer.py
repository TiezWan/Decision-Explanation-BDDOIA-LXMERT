...
class AOptimizer(DataLoader):
    ...
    def _loss_related_batch_loop(self, input: Tensor, target: Tensor, get_eval_type: str) -> float:
        ...
        elif get_eval_type == 'eval_train' or get_eval_type == 'val' or get_eval_type == 'test':
            pass
                ...
                with torch.no_grad():
                    ...
                    if args.save_heatmap:
                        for sent_nr, sent in enumerate(sents):
                            ...
                            if attention_scores.size() == torch.Size([self.batch_size, args.heads, QUERY_LENGTH+2, args.objects]):
                                heatmap = HeatmapVisualization(num_heads=args.heads, num_objs=args.objects, available_questions=[0,1,2], pooling="sum", \
                                                               input_path=args.input, img_save_path=args.heatmap_savepath, heatmap_labels=args.heatmap_labels)                     
                                conf_mat_batch_sent = heatmap.run_and_eval(img_id, attention_scores, sent_nr, sent, cluster_lenth=args.cluster_lenth, eps_dbscan=args.eps_dbscan)
                                conf_mat_batch = tuple([x + y for x, y in zip (conf_mat_batch, conf_mat_batch_sent)])
    ...
    def test(self) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            if args.save_heatmap:
                ...
                Document.docu_eval_hist(content=content, file_name="heatmap_visualization_hist")
            else:
                #Start testing
                ...
                avg_loss, conf_mat, f1_score, label_acc, bitwise_acc, trace, img_sents_pair = self.eval('test')
                # Document the evaluation result
                content = {'epoch': epoch+1,
    ...
    def eval(self, get_eval_type: str) -> Tuple:
        ...
        if get_eval_type == 'eval_train':
            ...
            if args.save_heatmap:
                ...
                batch_loss, pre_answ, true_answ, img_sents_pair = self._batch_loop('test', self.te_dataloader)
                avg_loss = sum(batch_loss) / len(batch_loss)
                conf_mat = self.compute_conf_mat(pre_answ, true_answ)
                f1_score = self.compute_f1_score(conf_mat)
                label_acc, bitwise_acc, trace = self.compute_acc(pre_answ, true_answ)
    ...
    def heatmap_eval(self, conf_mat) -> Tuple:
        # conf_mat = true_neg, false_pos, false_neg, true_pos
