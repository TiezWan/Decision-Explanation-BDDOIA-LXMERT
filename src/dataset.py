...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, file_idx: str) -> None:
        ...
        logger.info(
            f"Loading dataset for {self.file_idx}...Done! Dataset with {self.__len__()} samples."
        )
    ...
    def __getitem__(self, idx: int):
        ...
        if self.file_idx == "train" or self.file_idx == "val":
            ...
            label = torch.tensor(
                self.idx2label[idx][1][ques_idx + ques_appendix], dtype=torch.float32
            )
        # There are two datasets for testing, normal dataset is just for testing, another one is for heatmap visulization
            ...
            if args.save_heatmap:
                ...
                for ques_idx in avail_ques_idx:
                    # Append possibility of the Counter question types: 1, 3, 5
                    ques_appendix = random.choice((0, 1))
                    sent_content = random.choice(
                        self.queries[ques_types[ques_idx + ques_appendix]]
                    )
                    sent.append(sent_content)
                    label = np.append(
                        label, self.idx2label[idx][1][ques_idx + ques_appendix]
                    )
                ...
                sent = random.choice(self.queries[ques_types[ques_idx + ques_appendix]])
                label = torch.tensor(
                    self.idx2label[idx][1][ques_idx + ques_appendix],
                    dtype=torch.float32,
                )
        else:
            raise ValueError(f"Unknown evaluation type: {self.file_idx}")
        ...
        assert (
            obj_num == objects.shape[0] == len(boxes) == len(features)
        ), f"Size didn't match: obj_num = {obj_num}, objects.shape = {objects.shape}, len(boxes) = {len(boxes)}, len(features) = {len(features)}"
