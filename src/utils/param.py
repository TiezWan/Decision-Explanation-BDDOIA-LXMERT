...
def parse_args():
    ...
    parser.add_argument(
        "--heatmap_savepath", type=str, default="./output/visualization_images"
    )
    parser.add_argument(
        "--heatmap_labels",
        nargs="+",
    ...
    parser.add_argument(
        "--saveHeatmap", dest="save_heatmap", action="store_true", default=False
    )
    parser.add_argument(
        "--plotHeatmap", dest="plot_heatmap", action="store_true", default=True
    )
    parser.add_argument(
        "--eps_DBSCAN",
        dest="eps_dbscan",
    ...
    parser.add_argument(
        ...
        default=35,
        type=float,
        help="The allowed maximum number interest objects in a cluster",
    )
    # Model Loading
    parser.add_argument(
        "--load",
        type=str,
        default=None,
        help="Load specified weights, usually the fine-tuned weights for our task for testing.",
    )
    parser.add_argument(
        "--loadLxmert",
        dest="load_lxmert",
        type=str,
        default=None,
        help="Load pre-trained weights given by LXMERT.",
    )
    parser.add_argument(
        "--fromScratch",
        dest="from_scratch",
    ...
    parser.add_argument(
        ...
        default=100,
        type=int,
        help="Number of bounding boxes object",
    )
